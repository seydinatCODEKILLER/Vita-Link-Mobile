...
import { useCreateDay } from "@/src/hooks/useDonationDays";
...
type FormDataState = Partial<DayStep1Values> &
  Partial<DayStep2Values> & {
    targetDonors?: string; // string car vient du form, pas encore transformé
...
export default function CreateJourneeScreen() {
  ...
  const styles = useThemedStyles((c) => ({
    ...
    footer: {
      ...
      paddingBottom: 10,
      gap: 14,
      // Optionnel : ajoute un léger dégradé/fond pour que le texte défilant passe derrière proprement
      backgroundColor: colors.bg + "CC", // 👈 Ajoute ça (ou un linear gradient)
      paddingTop: 12, // 👈 Ajoute ça
    },
    ctaBtn: {
      flexDirection: "row",
      alignItems: "center",
  ...
  const form3 = useForm<DayStep3FormValues>({
    resolver: zodResolver(dayStep3Schema) as any,
    defaultValues: {
      targetDonors: "50",
      bloodTypesNeeded: [],
    },
  ...
  const handleNext = async () => {
    await currentForm.handleSubmit(
      (data) => {
        // ✅ Plus d'erreur : data de form1 et form2 sont compatibles avec FormDataState
        // data de form3 (DayStep3FormValues) l'est aussi car targetDonors: string
    ...
    await form3.handleSubmit(
      async (rawStep3Data) => {
        ...
        try {
          ...
          form1.reset();
          form2.reset();
          form3.reset({ targetDonors: "50", bloodTypesNeeded: [] });
          setImageUri(null);
          setCurrentStep(1);
          setFormData({});
            ...
            {isPending ? (
              ...
              <>
                <Text style={styles.ctaBtnText}>
                  {currentStep < 3 ? "Continuer" : "Publier l'événement"}
                </Text>
